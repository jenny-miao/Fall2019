...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        if isinstance(rest, MacroProcedure):
            rest.apply_macro(rest, env)
        ...
        operands = expr.rest.map(lambda elem: scheme_eval(elem, env))
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        last_expr = scheme_eval(expressions.first, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("number of arguments does not match number of formal parameters")
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(bind_pair, 2, 2)
        formals, values = Pair(bind_pair.first, formals), Pair(scheme_eval(bind_pair.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    macro.apply_macro(target.rest, env)
    return target.first
    # END Problem 20
