...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if hasattr(self.ant, 'is_container') and self.ant.is_container:
                    if BodyguardAnt.can_contain(self.ant, insect):
                        BodyguardAnt.contain_ant(self.ant, insect)

                    else:
                        assert self.ant is None, 'Two ants in {0}'.format(self)
                else:
                    if BodyguardAnt.can_contain(insect, self.ant):
                        BodyguardAnt.contain_ant(insect, self.ant)
                        self.ant = insect
                    else:
                        assert self.ant is None, 'Two ants in {0}'.format(self)                    
                # END Problem 9
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(self.ant) == QueenAnt and QueenAnt.trueQueen.index(self.ant) == 0:
                return
            elif type(self.ant).is_container and type(insect) == QueenAnt and QueenAnt.trueQueen.index(insect) == 0:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        entrance = self.place
        while entrance:
            if entrance.bees != [] and entrance is not beehive:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if original_place != self.place:
            bonus = self.damage
        for b in original_place.bees[:]:
            Insect.reduce_armor(b, amount + bonus)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            HungryAnt.eat_bee(self, random_or_none(self.place.bees))
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return self.contained_ant == None and other.is_container == False
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, armor)
        QueenAnt.trueQueen.extend([self])
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.trueQueen.index(self) == 0:
            ScubaThrower.action(self, colony)
            exit = self.place.exit
            while exit != None:
                if exit.ant != None:
                    if not exit.ant in QueenAnt.buffedAnts:
                        exit.ant.damage = (exit.ant.damage * 2)
                        QueenAnt.buffedAnts.extend([exit.ant])
                    if exit.ant.is_container:
                        if exit.ant.contained_ant != None and not exit.ant.contained_ant in QueenAnt.buffedAnts:
                            exit.ant.contained_ant.damage = (exit.ant.contained_ant.damage * 2)
                            QueenAnt.buffedAnts.extend([exit.ant.contained_ant])
                exit = exit.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Insect.reduce_armor(self, amount)
        if QueenAnt.trueQueen.index(self) == 0 and self.armor == 0:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    def decrement(colony):
        nonlocal duration
        if duration > 0:
            new_action(bee, colony)
        else:
            bee.direction = 1
            old_action(colony)
        duration -= 1

    bee.action = decrement
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and not target.scare:
